...
router.get("/me", getTokenFrom, authenticateToken, attachUser, ensureAuth, (req, res, next) => {
    const user = req.user;
    return res.status(200).json({ user });
});

// update user's profile fields
router.put("/me", getTokenFrom, authenticateToken, attachUser, ensureAuth, async (req, res, next) => {
    try {
